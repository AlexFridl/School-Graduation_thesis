...
//            dd($tmStmDatumPokupljeni);
            ...
            try {
                if ($tmStmDatumPokupljeni > $tmStmDatumPokupljeni_now || $tmStmDatumPokupljeni == $tmStmDatumPokupljeni_now) {
                    ...
                    if ($rez) {
                        return redirect()->route('zakazaniA')->with('poruka', 'Uspešno ste izvršili zakazivanje!');
                    } else {
                    ...
                    return redirect()->route('adminPanel_zakazano_insert')->with('poruka', 'Morate odabrati datum posle današnjeg datuma!');
                }
            }
            catch (\Exception $ex) {
...
//            $tipTretmana = $request->input('ddlTipTretman');
            ...
            try {
                if ($tmStmDatumPokupljeni > $tmStDanasnjiDatum || $tmStmDatumPokupljeni == $tmStDanasnjiDatum) {
                    ...
                    if ($rez) {
                        return redirect()->route('zakazaniA')->with('poruka', 'Uspešno ste izmenili zakazani termin!');
                    } else {
    ...
    public function adminPanel_zakazani_delete($id_zakazano)
    {
        ...
        try {
            if ($allZakazano->status_termina == 1) {
                ...
                return redirect()->route('zakazani')->with('poruka', 'Uspešno brisanje zakazanog termina!');
            }
        } catch (\Exception $ex) {
            \Log::error("Greska:" . $ex->getMessage());
        }
...
//            dd($day);
            if ($unos == $date) {
                ...
                if ($z->status_termina == '0') {
                    $podaci .= "<td><a  class='linkA'  href='".route('adminPanel_zakazani_delete',['id_zakazano'=>$z->id_zakazano])."'>Obriši</a>";
                } else {
                    $podaci .= "<td><a  class='linkA'  href='".route('adminPanel_zakazani_delete',['id_zakazano'=>$z->id_zakazano])."'>Otkaži</a>";
                }
                $podaci .= "</tr>";
            }
        }
...
//        dd($this->data['zakazani']);
    ...
    public function doOtkazaniA($id_zakazano,Request $request){
        ...
        if($rez)
        {
            return redirect()->route('otkazaniA')->with('poruka',"Uspešno ste obrisali otkazani termin!");
        }
        else{
            return redirect()->route('otkazaniA')->with('poruka',"Neuspešno brisanje termina!");
        }
